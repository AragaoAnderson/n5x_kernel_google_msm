...
if [ $USER != "root" ]; then
	make -j$NUMBEROFCPUS CONFIG_NO_ERROR_ON_MISMATCH=y zImage
else
	nice -n -15 make -j$NUMBEROFCPUS CONFIG_NO_ERROR_ON_MISMATCH=y zImage
fi;

if [ -e $KERNELDIR/arch/arm/boot/zImage ]; then
        echo "${bldcya}***** Final Touch for Kernel *****${txtrst}"
	rm $KERNELDIR/out/zImage >> /dev/null;
        cp $KERNELDIR/arch/arm/boot/zImage $KERNELDIR/out/zImage;
        stat $KERNELDIR/out/zImage || exit 1;
	
	echo "--- Creating boot.img ---"
	# copy all needed to out kernel folder
        ./utilities/mkbootimg --kernel $KERNELDIR/out/zImage --cmdline 'console=ttyHSL0,115200,n8 androidboot.hardware=mako lpj=67677 user_debug=31' --base 0x80200000 --pagesize 2048 --ramdisk_offset 0x01600000 --ramdisk ramdisk.gz --output $KERNELDIR/out/boot.img
